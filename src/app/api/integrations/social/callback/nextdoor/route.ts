...
export async function GET(req: Request) {
  ...
  if (attempt.state !== state || attempt.workspaceId !== workspaceId || attempt.platform !== "nextdoor") {
    return NextResponse.redirect(`${process.env.APP_URL}/onboarding?error=state_mismatch`);
  }

  // Exchange code for token (Nextdoor)
  const tokenParams = new URLSearchParams({
    client_id: process.env.NEXTDOOR_CLIENT_ID || "",
    client_secret: process.env.NEXTDOOR_CLIENT_SECRET || "",
    redirect_uri: process.env.NEXTDOOR_REDIRECT_URI || "",
    code,
    grant_type: "authorization_code",
  });

  const tokenRes = await fetch(`https://api.nextdoor.com/oauth/v2/token`, {
    method: "POST",
  ...
  const integrationId = `social_nextdoor_${workspaceId}`;
  ...
  await adminDb.collection("workspace_onboarding").doc(workspaceId).set(
    {
      "integrations.social.statusByPlatform.nextdoor": { status: "connected" },
      "integrations.social.connectedPlatforms": admin.firestore.FieldValue.arrayUnion("nextdoor"),
