...
export async function GET(req: Request) {
  ...
  if (attempt.state !== state || attempt.workspaceId !== workspaceId || attempt.platform !== "snapchat") {
    return NextResponse.redirect(`${process.env.APP_URL}/onboarding?error=state_mismatch`);
  }

  // Exchange code for token (Snapchat)
  ...
  const integrationId = `social_snapchat_${workspaceId}`;
  ...
  await adminDb.collection("workspace_onboarding").doc(workspaceId).set(
    {
      "integrations.social.statusByPlatform.snapchat": { status: "connected" },
      "integrations.social.connectedPlatforms": admin.firestore.FieldValue.arrayUnion("snapchat"),
